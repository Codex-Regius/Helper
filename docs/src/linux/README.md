...
Liste des sessions screens
`screen -ls`

Lancer une session screen
`screen -s <id_optionnel>`

Récupérer une sesion screen avec <id_optionnel>
`screen -r <id_optionnel>`

Se détacher du screen courant : combinaison de touches Ctrl + A + D

Detacher d'un screen
`screen -d <id_optionnel>`
